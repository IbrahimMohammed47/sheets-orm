...
type NumberOperators = BaseOperator<number> & {
    gt?: number;
    lt?: number;
    gte?: number;
    lte?: number;
};
type StringOperators = BaseOperator<string> & {
    contains?: string;
    like?: string;
    startsWith?: string;
    endsWith?: string;
...
type SheetFieldKindToOperators<SheetFieldKind> = SheetFieldKind extends "STRING"
    ? StringOperators
...
type SchemaToFilters<T extends readonly SheetField[]> = {
    [K in T[number] as K["isDeleted"] extends true
        ? never
        : K["name"]]: SheetFieldKindToOperators<K["kind"]>;
};

type SchemaToSelections<T extends readonly SheetField[]> =
    (keyof SchemaToFilters<T>)[];
...
type Filters<T extends readonly SheetField[]> = Partial<SchemaToFilters<T>> &
    Partial<DefaultFilters> &
    QueryExpression<T>;

export type FindManyArgs<T extends readonly SheetField[]> = {
    filters?: Filters<T>;
    selections?: SchemaToSelections<T>;
    limit?: number;
    offset?: number;
};
function sqlizeBaseOperators(
    colName: keyof typeof SheetCols,
    operators: BaseOperator<any>,
    quote: boolean = true
) {
    const operations: string[] = [];
    if (operators.eq != null) {
        operations.push(
            `${colName} = ${quote ? '"' : ""}${operators.eq}${quote ? '"' : ""}`
        );
    }
    if (operators.neq != null) {
        operations.push(
            `${colName} != ${quote ? '"' : ""}${operators.neq}${
                quote ? '"' : ""
            }`
        );
    }
...
function sqlizeBooleanOperators(
    colName: keyof typeof SheetCols,
    operators: BooleanOperators
) {
    const operations: string[] = [
        ...sqlizeBaseOperators(colName, operators, false),
    ];
    return operations;
}
function sqlizeNumberOperators(
    colName: keyof typeof SheetCols,
...
function sqlizeStringOperators(
    colName: keyof typeof SheetCols,
    operators: StringOperators
) {
    const operations: string[] = [
        ...sqlizeBaseOperators(colName, operators, true),
    ];
...
function sqlizeDateOperators(
    ...
    if (operators.eq != null) {
        operations.push(
            `${colName} = datetime "${jsDateToSheetDateStr(operators.eq)}"`
        );
    }
    if (operators.neq != null) {
        operations.push(
            `${colName} != datetime "${jsDateToSheetDateStr(operators.neq)}"`
        );
    }
    if (operators.after != null) {
        operations.push(
            `${colName} > datetime "${jsDateToSheetDateStr(operators.after)}"`
        );
    }
    if (operators.before != null) {
        operations.push(
            `${colName} < datetime "${jsDateToSheetDateStr(operators.before)}"`
        );
    }
    if (operators.isNull === true) {
        operations.push(`${colName} is null`);
...
function parseFilters<T extends readonly SheetField[]>(
    filters: Filters<T>,
    schema: readonly [...typeof defaultFields, ...T]
): string {
    ...
    for (const [key, operators] of tuples) {
        if (key === "AND" && Array.isArray(operators)) {
            ...
            if (schemaField.kind === "BOOLEAN") {
                operations.push(
                    andClause(
                        sqlizeBooleanOperators(
                            schemaField.mapsTo,
                            operators as BooleanOperators
                        )
                    )
                );
                ...
                operations.push(
                    andClause(
                        sqlizeNumberOperators(
                            schemaField.mapsTo,
                            operators as NumberOperators
                        )
                    )
                );
                ...
                operations.push(
                    andClause(
                        sqlizeStringOperators(
                            schemaField.mapsTo,
                            operators as StringOperators
                        )
                    )
                );
...
function parseSelectors<T extends readonly SheetField[]>(
    filters: SchemaToSelections<T>,
    schema: readonly [...typeof defaultFields, ...T]
): string {
    const mapped = filters.map(
        (filter) => schema.find((f) => f.name === filter)?.mapsTo ?? ""
    );
    return mapped.join(", ");
}

export function parseQuery<T extends readonly SheetField[]>(
    query: FindManyArgs<T>,
    schema: readonly [...typeof defaultFields, ...T]
): string {
    ...
    if (selections && selections.length > 0) {
        queryString += `${parseSelectors(selections, schema)}\n`;
    } else {
        queryString += `${schema.map((x) => x.mapsTo).join(", ")} \n`;
    }
    if (filters) {
